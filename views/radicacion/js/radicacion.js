...
$(document).ready(function () {
	...
	$('body').on('change', 'select[name="renombramiento_tipo_documento"]', function () {
		/**
		 * Obtengo los valores de radicacion como el numero de documento y el tipo de proceso de cada cliente
		 */
			...
			if (xhr.getResponseHeader("content-type") == 'text/html; charset=UTF-8') {
				...
				$('button#nuevo-radicado').on('click', function () {
					if (confirm("Esta seguro de crear una nueva radicación?")) {
						...
						/**
						 * Adjunta los eventos correpondientes a todos los campos de llenado de número de planilla
						 * @param {array} controlDiv - corresponde al contenedor principal de los controles de llenado de número de planilla y al
						 * campo final en el cual se va a mostrar toda el número de planilla
						 */
						(function eventosNumeroPlanilla(controlDiv) {
							for (let i = 0; i < controlDiv.length; i += 2) {
								var controles = controlDiv[i];
							...
							$('#controls_num_planilla'), $('#numero_planilla')
						]);
						...
						var cambiaValorInputNumPlanilla = function (input) {
							...
							if (input.attr('id') == 'numero_planilla') {
								controlesForm = $('#controls_num_planilla');
							}
				...
				$('button#guardar_radicacion').on('click', function (event) {
					...
					$.ajax({
						...
						error: function (xhr) {
							...
							AlertMessage(STATES_ERROR, 'RESPUESTA CLIENTE ERROR!!!', xhr.responseText, {
								hideAfter: false
							});
				...
				if (error.type == 'error') {
					...
					AlertMessage(STATES_ERROR, 'ERROR !!!', error.message, {
						hideAfter: false
					});
					...
					if (confirm(error.message)) {
						...
						$.ajax({
							...
							data: {
								tipo_documento: error.tipo_documento,
								documentClient: error.id
							},
...
ListRadicacion = function (cliente) {
	...
	$.ajax({
		...
		error: function (xhr) {
			AlertMessage(STATES_ERROR, 'ERROR!!!', "No se ha podido visualizar las radicaciones del cliente.", {
				hideAfter: false,
				stack: false
			});
			console.log(xhr);
...
function EditRadicacion(el, id) {
	...
	if (continuar) {
		$.ajax({
			...
			error: function (xhr) {
				AlertMessage(STATES_ERROR, 'ERROR!!!', "Ha ocurrido un error al visualizar la radicación.", {
					hideAfter: false
				});
				console.log(xhr.responseText);
			}
	...
	if (confirm("Está seguro que desea eliminar esta radicación?")) {
		...
		$.ajax({
			...
			success: function (response) {
				if (!response.error) {
					row.remove();
					AlertMessage(STATES_OK, 'EXITO!!!', "Se ha eliminado Correctamente la radicación No. " + radicacionNumber, {
						stack: false,
						hideAfter: 1500,
						afterHidden: function () {
							selected_radicacion = false;
					...
					AlertMessage(STATES_ERROR, 'ERROR!!!', "Ha ocurrido un error al eliminar la radicación No. " + radicacionNumber + ". ERROR: [" + response.message + "]", {
						hideAfter: false,
						stack: false
					});
				}
			...
			error: function (xhr) {
				console.log(xhr);
				AlertMessage(STATES_ERROR, 'ERROR!!!', "Ha ocurrido un error al eliminar la radicación No. " + radicacionNumber, {
					hideAfter: false,
					stack: false
				});
			}
...
function DeleteCliente(el, id) {
	if (id != undefined) {
			...
			$.ajax({
				...
				success: function (response) {
					if (response.error != undefined) {
						AlertMessage(STATES_ERROR, 'ERROR!!!', "Ha ocurrido un error al intentar eliminar el cliente. ERROR: [" + response.message + "]", {
							hideAfter: false,
							stack: false
						});
					} else {
				...
				error: function (xhr) {
					console.log(xhr);
					AlertMessage(STATES_ERROR, 'ERROR!!!', "Ha ocurrido un error al intentar eliminar el cliente.", {
						hideAfter: false,
						stack: false
					});
				}
		...
		AlertMessage(STATES_ERROR, 'ERROR!!!', "No se ha encontrado un cliente para eliminar. ", {
			hideAfter: false,
			stack: false
		});
	}
